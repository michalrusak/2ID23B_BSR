...
class PythonBlockchainChunkStore {
	constructor(imageId, totalLength, pieceLength) {
		...
		console.log(`[PythonBlockchainChunkStore] Python backend URL: ${this.pythonBackendUrl}`)
	}
	...
	async _fetchAndCacheImageData() {
		...
		if (!this.imageDataCache) {
			console.log(`[PythonBlockchainChunkStore] Fetching image data for ${this.imageId} from Python blockchain...`)
			
			try {
				// Najpierw pobierz cały blockchain
				const chainResponse = await axios.get(`${this.pythonBackendUrl}/blockchain/chain`, {
					timeout: 10000,
				})
				...
				console.error(`[PythonBlockchainChunkStore] Error fetching image data: ${error.message}`)
				throw error
			}
		}
	...
	get(pieceIndex, options, callback) {
		...
		console.log(`[PythonBlockchainChunkStore] GET request for pieceIndex: ${pieceIndex} for imageId: ${this.imageId}`)

		this._fetchAndCacheImageData()
			.then(imageData => {
				if (!imageData) {
					console.error(`[PythonBlockchainChunkStore] No image data available for ${this.imageId}`)
					return callback(new Error(`No image data available for ${this.imageId}`))
				}

				const startOffset = pieceIndex * this.pieceLength
				const endOffset = Math.min(startOffset + this.pieceLength, imageData.length)
				
				if (startOffset >= imageData.length) {
					console.log(`[PythonBlockchainChunkStore] Piece ${pieceIndex} is beyond data length`)
					return callback(new Error('Piece index out of bounds'))
				}

				const pieceData = imageData.slice(startOffset, endOffset)
				console.log(
					`[PythonBlockchainChunkStore] Returning piece ${pieceIndex}: ${pieceData.length} bytes (${startOffset}-${endOffset-1})`
				)
				callback(null, pieceData)
			})
			.catch(err => {
				console.error(`[PythonBlockchainChunkStore] Error in GET for piece ${pieceIndex}:`, err)
				callback(err)
			})
	}
	...
	// Zamknij store
	close(callback) {
		console.log(`[PythonBlockchainChunkStore] CLOSE called for imageId: ${this.imageId}`)
		this.closed = true
	...
	// Zniszcz store
	destroy(callback) {
		console.log(`[PythonBlockchainChunkStore] DESTROY called for imageId: ${this.imageId}`)
		this.close(callback)
	}
}

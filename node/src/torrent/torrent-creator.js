	...
	async generateTorrentForImage(imageFilePath, imageId) {
		try {
			...
			try {
				...
				console.log(`[TorrentCreator] Successfully deleted temporary file: ${imageFilePath}`)
			} catch (unlinkError) {
				console.error(`[TorrentCreator] Error deleting temporary file ${imageFilePath}: ${unlinkError.message}`)

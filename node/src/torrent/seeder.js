...
const BlockchainChunkStore = require('./BlockchainChunkStore') // Zaimportuj nowy magazyn

class TorrentSeeder {
	constructor() {
		this.client = new WebTorrent()
		this.torrentsPath = path.resolve(process.cwd(), config.get('storage.torrentsPath'))
		...
		console.log(`[TorrentSeeder] Torrents path for .torrent files: ${this.torrentsPath}`)
		...
		this.client.on('torrent', torrent => {
			console.log(`[TorrentSeeder] Torrent added: ${torrent.infoHash} - ${torrent.name}`)
			console.log(`[TorrentSeeder] Torrent files: ${torrent.files.map(f => f.name).join(', ')}`)
			console.log(`[TorrentSeeder] Torrent trackers: ${torrent.announce}`)
			torrent.on('ready', () => {
				console.log(`[TorrentSeeder] Torrent ready to seed: ${torrent.infoHash} - ${torrent.name}`)
				console.log(`[TorrentSeeder] Seeders: ${torrent.numPeers}`) // numPeers includes seeders and leechers
			})
			torrent.on('warning', err => {
				console.warn(`[TorrentSeeder] Torrent warning for ${torrent.infoHash}:`, err)
			})
			torrent.on('error', err => {
				console.error(`[TorrentSeeder] Torrent error for ${torrent.infoHash}:`, err)
			})
			torrent.on('download', bytes => {
				// console.log(`[TorrentSeeder] Torrent ${torrent.infoHash} downloaded: ${bytes} bytes`);
			})
			torrent.on('upload', bytes => {
				console.log(`[TorrentSeeder] Torrent ${torrent.infoHash} uploaded: ${bytes} bytes`) // ODKOMENTOWANO
			})
			torrent.on('wire', function (wire, addr) {
				console.log(`[TorrentSeeder] Connected to peer ${wire.peerId} at ${addr} for torrent ${torrent.infoHash}`)
			})
		})
	}
	...
	async seedFileFromBlockchain(torrentFilePath, imageId) {
		...
		try {
			if (!fs.existsSync(torrentFilePath)) {
				console.error(`[TorrentSeeder] Torrent file not found at: ${torrentFilePath}`)
			...
			if (!parsedTorrent || !parsedTorrent.length || !parsedTorrent.pieceLength) {
				...
				}, Length: ${parsedTorrent ? parsedTorrent.length : 'N/A'}, PieceLength: ${
					parsedTorrent ? parsedTorrent.pieceLength : 'N/A'
				}`
				console.error(errMsg)
				throw new Error(errMsg)
			}
			const blockchainNodes = config.get('network.nodes')

			// Utworz klasę magazynu specyficzną dla tego torrent
			class TorrentSpecificStore extends BlockchainChunkStore {
				constructor(chunkLength, storeOpts) {
					console.log(`[TorrentSeeder] TorrentSpecificStore constructor called. chunkLength: ${chunkLength}`)
					console.log(`[TorrentSeeder] storeOpts:`, storeOpts)
					// Przekaż parametry do nadklasy
					super(imageId, parsedTorrent.length, chunkLength, blockchainNodes)
				}
			}
			...
			console.log(`[TorrentSeeder] Adding torrent to client with TorrentSpecificStore for imageId: ${imageId}`)
			...
			this.client.add(torrentFileBuffer, options, torrent => {
				...
				torrent.files.forEach(file => {
					console.log(`  - ${file.name} (path in torrent: ${file.path}, length: ${file.length})`)
				})
				console.log(`[TorrentSeeder] Torrent announce URLs: ${torrent.announce.join(', ')}`)
				console.log(`[TorrentSeeder] Torrent magnet URI: ${torrent.magnetURI}`)
			})
		} catch (error) {
			console.error(`[TorrentSeeder] Error seeding file from blockchain: ${error.message}`, error)
	...
	async seedTorrentByInfoHash(torrentId, filePath) {
		return new Promise(async (resolve, reject) => {
			try {
				...
				this.client.add(torrentId, { path: path.dirname(filePath) }, torrent => {
